...
localtime4 = time.strftime("%Y-%m-%d %W %w %j %p %x %X %Z %H:%M:%S", time.localtime())
print(localtime3)
print(localtime4)
# 格式化成Sat Mar 28 22:24:24 2016形式
print(time.strftime("%a %b %d %H:%M:%S", time.localtime()))
# 将格式字符串转换为时间戳
a = "Sat Mar 28 22:24:24 2021"
print(time.mktime(time.strptime(a, "%a %b %d %H:%M:%S %Y")))
b = "2021-12-28 22:24:24 "
...
print(calendar.prmonth(2021, 12, w=0, l=0))
print(calendar.monthrange(2021, 11))
print(calendar.weekday(2021, 12, 10))


# Time 模块
"""Time 模块包含了以下内置函数，既有时间处理的，也有转换时间格式的："""
# 1.ime.altzone 返回格林威治西部的夏令时地区的偏移秒数。如果该地区在格林威治东部会返回负值（如西欧，包括英国）。对夏令时启用地区才能使用。
import time
print("time.altzone %d " % time.altzone)

# 2.time.asctime([tupletime]) 接受时间元组并返回一个可读的形式为"Tue Dec 11 18:07:14 2008"（2008年12月11日 周二18时07分14秒）的24个字符的字符串。
import time
print("time.asctime(t): %s " % time.asctime(time.localtime()))

# 3.time.clock()  用以浮点数计算的秒数返回当前的CPU时间。用来衡量不同程序的耗时，比time.time()更有用
# 实例 由于该方法依赖操作系统，在 Python 3.3 以后不被推荐，而在 3.8 版本中被移除，需使用下列两个函数替代。
...
# >>>print(calendar.isleap(2000)) # True   >>> print(calendar.isleap(1900)) # False
